...
import {getAllWorkshopSchema,createWorkshopSchema,editWorkshopSchema,deleteWorkshopSchema} from "../../validators/Workshop/index"
...
export const GetWorkshop = async (req: Request, res: Response) => {
   try {
      ...
      return res.status(200).json({ message: 'Workshop fetched successfully',data:AllWorkshops });
    } catch (err) {
...
export const CreateWorkshop = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const UploadedData={
        posterName:"SHUKLA",
        posterLink:"https://fastly.picsum.photos/id/325/600/400.jpg?hmac=Zxo0NO4DZWEMHx7MHHwfRuTbnyiS9OX99fpfudirzaE",
        posterType:"image/jpeg"
      }
      const NewWorkShopData:any={
        title,instruction,mode,date,time,posterImage:UploadedData
      }
      if(mode==="Offline"){
...
export const EditWorkshop = async (req: Request, res: Response) => {
    try {
      ...
      await editWorkshopSchema.validate(body,
        { abortEarly: false }
      );
      const { workshopId,title,instruction,address,mode,date,time,posterImage}=body
