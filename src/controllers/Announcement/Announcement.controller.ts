...
} from '../../validators/Announcement/index';
...
export const GetAnnouncement = async (req: Request, res: Response) => {
  try {
    ...
    await getAnnouncementSchema.validate(
      { page, limit },
      { abortEarly: false }
    );
...
export const CreateAnnouncement = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Sucessfully created announcement' });
  } catch (err) {
    if (err instanceof yup.ValidationError) {
      return res.status(400).json({ error: err.errors });
    }
...
export const EditAnnouncement = async (req: Request, res: Response) => {
  try {
    ...
    if (!UpdatedAnnouncement) {
      ...
      return res
        .status(200)
        .json({ message: 'Sucessfully edited announcement' });
    }
  } catch (err) {
    if (err instanceof yup.ValidationError) {
      return res.status(400).json({ error: err.errors });
...
export const DeleteAnnouncement = async (req: Request, res: Response) => {
  try {
    ...
    const DeleteAnnouncement = await AnnouncementModel.findOneAndDelete({
      _id: announcementId,
    });
    if (!DeleteAnnouncement) {
      ...
      return res
        .status(200)
        .json({ message: 'Delete announcement sucessfully' });
    }
  } catch (err) {
    if (err instanceof yup.ValidationError) {
      return res.status(400).json({ error: err.errors });
